    ...
    if not projetos:
        ...
        if 'projeto_selecionado' in st.session_state:
            ...
            if projeto:
                ...
                with col1:
                    if st.button("📋 Gerar Inventário PGR (NR-1)", use_container_width=True, disabled=not projeto.pode_gerar_pgr()):
                        # Aqui vamos integrar o gerador quando tivermos dados
                        st.info("Funcionalidade em desenvolvimento - requer dados consolidados")
                
                with col2:
                    if st.button("📊 Gerar AEP (NR-17)", use_container_width=True, disabled=not projeto.pode_gerar_pgr()):
                        st.info("Funcionalidade em desenvolvimento - requer dados consolidados")
                
                with col3:
                    if st.button("📈 Relatório Executivo", use_container_width=True, disabled=not projeto.pode_gerar_pgr()):
                        st.info("Funcionalidade em desenvolvimento - requer dados consolidados")
