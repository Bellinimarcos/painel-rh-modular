...
def detect_file_format(df):
    ...
    resp_cols = [col for col in df.columns if 'Resp_Q' in str(col) or (str(col).startswith('P') and str(col)[1:].isdigit())]
...
def render_results(analysis):
    ...
    with col2:
        high_risk = sum(1 for s in scores.values() if s < 40)
        st.metric("Dimensões de Alto Risco", high_risk)
    
    with col3:
        low_risk = sum(1 for s in scores.values() if s > 70)
    ...
    with col2:
        ...
        bottom5 = df_scores.nsmallest(5, 'Score')
...
# Botão de análise
if st.button("🚀 Executar Análise COPSOQ III", type="primary", use_container_width=True):
