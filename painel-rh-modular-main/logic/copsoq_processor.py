...
# Versão: Atualizada com suporte flexível a prefixos (P, Q, Resp_Q, e Q1 - texto do Forms)
# e limpeza de dados.
...
class COPSOQProcessor:
    ...
    def _setup_scales(self):
        ...
        if self.version == "III":
            ...
            # Confirmado por você: inverter apenas Q59 e Q60; não inverter Q58.
            ...
            self.scale_map = {
                "Ritmo de Trabalho": [1, 2],
                "Exigências Cognitivas": [3, 4],
            }
            self.inverted_idx = []
    ...
    def _normalize_google_forms_columns(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Converte cabeçalhos do Google Forms no padrão:
        ...
        for col in data.columns:
            c = str(col).strip()
            match = re.match(r"^Q(\d+)\s*[-–]\s*", c)
            if match:
                qnum = match.group(1)
                rename_map[col] = f"Resp_Q{qnum}"
    ...
    def _get_question_cols(self, columns):
        """
        Detecta colunas de perguntas nos formatos:
        - P1, P2...
        - Q1, Q2...
        - Resp_Q1...
    ...
    def _detect_prefix(self, columns) -> str:
        """
        Determina o prefixo predominante após normalização.
        Preferência:
        1. Resp_Q
        2. Q
        3. P
        """
        col_strings = [str(c).strip() for c in columns]

        if any(c.startswith("Resp_Q") and c[6:].isdigit() for c in col_strings):
            return "Resp_Q"
        if any(c.startswith("Q") and c[1:].isdigit() for c in col_strings):
            return "Q"
        return "P"
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        data = self._normalize_google_forms_columns(data)

        question_cols = self._get_question_cols(data.columns)

        if len(data) < 5:
            errors.append(f"Volume insuficiente ({len(data)} respostas). Mínimo de 5 exigido.")

        if len(question_cols) < 10:
            errors.append(f"Poucas colunas de perguntas ({len(question_cols)}) detectadas.")

        prefix = self._detect_prefix(data.columns)
        self.scales = {scale: [f"{prefix}{i}" for i in idx] for scale, idx in self.scale_map.items()}
        self.inverted_items = [f"{prefix}{i}" for i in self.inverted_idx]

        coverage = (
            sum(
                1
                for _, items in self.scales.items()
                if any(item in data.columns for item in items)
            )
            / len(self.scales)
            * 100
            if self.scales
        ...
        if coverage < 30:
            errors.append(
                f"Cobertura de escalas crítica ({coverage:.1f}%). Verifique os nomes das colunas."
            )
    ...
    def process(self, data: pd.DataFrame, name: str) -> AnalysisResult:
        ...
        for scale, items in self.scales.items():
            ...
            for item in items:
                if item in data.columns:
                    if fmt == "numeric":
                        num = pd.to_numeric(data[item], errors="coerce")
                    else:
                        num = data[item].apply(self._normalize_text).map(self.text_to_score)
            ...
            if vals:
                results[scale] = float(np.mean(vals))
        ...
        return AnalysisResult(
            ...
            metadata={
                ...
                "coverage": validation.quality_score,
            },
            risk_level=risk,
        )
    ...
    def _calculate_risk_level(self, results: dict) -> RiskLevel:
        scores = [
            (100 - score) if scale in self.positive_scales else score
